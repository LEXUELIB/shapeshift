      ...
      <div className="flex flex-col gap-3">
        ...
        <Field index={1} className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-1">
            <Button size="icon-sm" variant="ghost" aria-label={t("Fewer people")} disabled={!interactive || n <= 1} onClick={() => setPeople(Math.max(1, n - 1))}>
              <Minus />
            </Button>
            <span className="w-8 text-center text-[17px] font-[550] tabular-nums">{n}</span>
            <Button size="icon-sm" variant="ghost" aria-label={t("More people")} disabled={!interactive || n >= 99} onClick={() => setPeople(n + 1)}>
              <Plus />
            </Button>
          </div>
        </Field>
